...
2. Extract: which skills ran, what artifacts were created, and any **open threads**.

## Scan files

1. Check `files/` for any new or recently modified files.
2. Check `/_workspace/references/` for registered references.
3. Check `/_workspace/skills/` for available user skills.
...
If there are existing user skills or files despite no setup, mention those too — the user may have been working manually.
...
3. **Available skills** — suggest the most relevant ones based on recent activity
4. **General options** — "add a reference", "create a new skill", etc.

## If there's nothing to go on

If there are no logs, no files, and no skills (but the workspace is set up), propose:
- Try one of the available skills (list them)
- Add reference material to make skills smarter
- Create a new skill
