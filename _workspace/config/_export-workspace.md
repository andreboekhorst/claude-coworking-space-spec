...
>
> 1. **Clean template** — A reusable workspace others can clone. Strips all personal data (logs, context files). Keeps skills, config, and structure.
...
- `_workspace/skills/` (all skill files)
...
This is a **Claude Workspace** — a structured folder of skills, references, and templates that Claude uses to assist you. Open this folder in your editor with Claude Code, and Claude knows how to help.
...
|--------|-------------|---------------|
[For each skill in _workspace/skills/, add a row with name, description from frontmatter, and trigger example]
...
| Folder | Purpose |
|--------|---------|
| `_workspace/config/` | System skills (don't edit) |
| `_workspace/skills/` | User-facing skills |
| `_workspace/references/` | References and user settings (filled during setup) |
| `_workspace/logs/` | Session logs |
```

Populate the skills table by reading the frontmatter (`name`, `description`, `trigger`) from each file in `_workspace/skills/`. Only include user skills, not system config skills.
